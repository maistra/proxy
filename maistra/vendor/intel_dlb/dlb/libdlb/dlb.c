    ...
    dlb_shared_domain_t *shared_domain)
{
    ...
    domain_hdl = malloc(sizeof(*domain_hdl));
    ...
    int port_id)
{
    ...
    * result in the four levels specified in dlb_queue_depth_levels_t.
    */
    int threshold = domain->shared->sw_credits.total_credits[DIR] * 2 / 3;
    ...
    int priority)
{
    ...
    CHECK(priority >= 0 && priority <= 7, EINVAL);
    ...
    void *callback_arg)
{
    ...
    /* Freed by the child thread */
    domain = malloc(sizeof(dlb_domain_t));
...
static inline int num_token_pops(
    ...
    bool is_2_5)
{
     ...
     * to the domain's pool.
     */
    int batch_size = DLB_SW_CREDIT_BATCH_SZ;

    if (port_hdl->port.shared->credits[LDB].num >= 2 * batch_size) {
        __sync_fetch_and_add(port_hdl->credit_pool[LDB], batch_size);
        port_hdl->port.shared->credits[LDB].num -= batch_size;
    }

    if (is_2_5)
        return;

    if (port_hdl->port.shared->credits[DIR].num >= 2 * batch_size) {
        __sync_fetch_and_add(port_hdl->credit_pool[DIR], batch_size);
        port_hdl->port.shared->credits[DIR].num -= batch_size;
    }
}
...
cleanup:
    ...
    dlb2_release_port_credits(port_hdl, is_2_5);
...
cleanup:
    ...
    dlb2_release_port_credits(port_hdl, false);
...
cleanup:
    dlb2_release_port_credits(port_hdl, true);
    ...
    dlb_recv_t *event)
{
    ...
    if (port_hdl->event_fd && PORT_CQ_IS_EMPTY(port_hdl, port)) {
	    ret = read(port_hdl->event_fd, &reset_ctr, sizeof(reset_ctr));
	    if (ret < 0)
    ...
    if (port_hdl->device_version.ver == VER_DLB2)
        dlb2_inc_port_credits(port, i, false);
    else if (port_hdl->device_version.ver == VER_DLB2_5)
        dlb2_inc_port_credits(port, i, true);
