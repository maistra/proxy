...
bool dlb2_port_probe = false;
module_param_named(port_probe, dlb2_port_probe, bool, 0444);
MODULE_PARM_DESC(port_probe, "Probe DLB2 ports for best port selection");
				     ...
				     u32 domain_id)
{
	if (port->valid &&
		...
		dma_free_coherent(&dlb2->pdev->dev,
				  DLB2_CQ_SIZE,
				  port->cq_base,
				  port->cq_dma_base);
