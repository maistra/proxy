	...
	for (i = 0; i < DLB2_VF_TOTAL_NUM_INTERRUPT_VECTORS; i++) {
		...
		if (dlb2->intr.msi_map[i].is_ldb) {
			...
			if (primary_vf->ldb_port[port_id].efd_ctx)
				ret = eventfd_signal(primary_vf->ldb_port[port_id].efd_ctx, 1);
			else {
			...
			if (primary_vf->dir_port[port_id].efd_ctx)
				ret = eventfd_signal(primary_vf->dir_port[port_id].efd_ctx, 1);
			else {
		...
		if (ret != 1) {
			dev_err(dlb2->dev, "[%s()] port %d eventfd signal failed\n",
				__func__, port_id);
			return ret;
		}
	}
	...
	if (vector == DLB2_INT_NON_CQ) {
		...
		if (dlb2->intr.msi_map[idx].is_ldb) {
			...
			if (dlb2->ldb_port[port_id].efd_ctx)
				ret = eventfd_signal(dlb2->ldb_port[port_id].efd_ctx, 1);
			else {
			...
			if (dlb2->dir_port[port_id].efd_ctx)
				ret = eventfd_signal(dlb2->dir_port[port_id].efd_ctx, 1);
			else {
