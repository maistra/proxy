...
ContextImpl::ContextImpl(Stats::Scope& scope, const Envoy::Ssl::ContextConfig& config,
  ...
  if (!capabilities_.provides_ciphers_and_curves &&
    ...
    if (!capabilities_.provides_sigalgs && !config.signatureAlgorithms().empty()) {
      if (!SSL_CTX_set1_sigalgs_list(tls_context_.ssl_ctx_.get(), config.signatureAlgorithms().c_str())) {
...
ServerContextImpl::ServerContextImpl(Stats::Scope& scope,
 ...
 #if 0  // dcillera TODO: postpone this code
  for(auto& ctx : tls_context_.cert_contexts_) {
...
#if 0  // TODO: dcillera - to be added in Maistra
...
void ServerContextImpl::populateServerNamesMap(CertContext& ctx, int pkey_id) {
  if (ctx.cert_chain_ == nullptr) {
    return;
  }
  ...
  auto populate = [&](const std::string& sn) {
    ...
    sn_match->second.emplace(std::pair<int, std::reference_wrapper<CertContext>>(pkey_id, ctx));
  };
...
#if 0 // TODO dcillera: have we to port this into Maistra using OpenSSL?
